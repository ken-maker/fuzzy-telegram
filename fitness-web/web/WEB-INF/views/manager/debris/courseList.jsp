...
<div id="light">
    ...
    <form id="addVIP_form" class="layui-form">
        <input id="name" placeholder="课程名"/><span id="name_remark"></span>
...
<table class="layui-table">
    ...
    <c:forEach var="course" items="${courseList.list}">
        <tr class="layui-table-body">
            ...
            <td>${course.nowPeople}</td>
            <td>${course.picture}</td>
            <td>${course.classTime}</td>
            <td>
                <button class="layui-btn-danger layui-btn-sm">
                    <a id="delete_a" href="${pageContext.request.contextPath}/courseController/deleteCourse?id=${course.id}">删除</a>
                </button>
                <button class="layui-btn-warm layui-btn-sm">
                    <a href="${pageContext.request.contextPath}/courseController/updateCourse?id=${course.id}">修改</a>
                </button>
            </td>
        </tr>
    </c:forEach>
...
<ul class="pagination">
    ...
    <li><a href="${pageContext.request.contextPath}/courseController/courseList?pageNum=${courseList.prePage}">&lt;&lt;上一页</a></li>
    ...
    <li><a href="${pageContext.request.contextPath}/courseController/courseList?pageNum=${courseList.nextPage}">下一页&gt;&gt;</a></li>
    <li><a href="${pageContext.request.contextPath}/courseController/courseList?pageNum=${courseList.pages}">尾页</a></li>
</ul>
    ...
    function addCourse(){
        $("#add_btn").click(function () {
            ...
            if(base(name,peopleCount,picture,nowPeople,classTime)) {
                alert("通过基本验证");
                $.ajax({
                    url:"${pageContext.request.contextPath}/courseController/add",
                    method:"GET",
                    data:{
                        name:name,
                        type:type,
